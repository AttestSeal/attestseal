...
Recommended: in the Tailscale admin console, give the PC a stable hostname (e.g. `ats-gaming-pc`) so future config doesn't tie to the IP if it changes.
...
scp "/Users/admin/Library/Mobile Documents/com~apple~CloudDocs/robots/attestseal/crawler/fetch_service.py" \
    Administrator@<GAMING_PC_TS_IP>:C:\ats-crawler\fetch_service.py
```
...
Create `C:\ats-crawler\crawler.env` with this content (use the same CRAWLER_SHARED_SECRET as the Mac Air so the API box's single secret works for both endpoints):
...
Create `C:\ats-crawler\start.ps1`:

```powershell
# Load environment from crawler.env, then launch the service
Get-Content C:\ats-crawler\crawler.env | ForEach-Object {
    if ($_ -match '^\s*([A-Z_]+)\s*=\s*(.+?)\s*$') {
        [Environment]::SetEnvironmentVariable($matches[1], $matches[2], "Process")
    }
}
Set-Location C:\ats-crawler
...
Install the service:
```powershell
nssm install ats-crawler "C:\ats-crawler\venv\Scripts\python.exe" "-m" "uvicorn" "fetch_service:app" "--host" "0.0.0.0" "--port" "8901"
nssm set ats-crawler AppDirectory "C:\ats-crawler"
nssm set ats-crawler AppStdout "C:\ats-crawler\stdout.log"
nssm set ats-crawler AppStderr "C:\ats-crawler\stderr.log"
nssm set ats-crawler AppRotateFiles 1
nssm set ats-crawler AppRotateBytes 10485760
nssm set ats-crawler Start SERVICE_AUTO_START
nssm set ats-crawler AppRestartDelay 5000

# Load env vars into the service's environment
$env_vars = Get-Content C:\ats-crawler\crawler.env | Where-Object { $_ -match '^\s*[A-Z_]+\s*=' } | ForEach-Object { $_.Trim() }
nssm set ats-crawler AppEnvironmentExtra $env_vars

nssm start ats-crawler
nssm status ats-crawler   # should print SERVICE_RUNNING
```
...
To check logs:
```powershell
Get-Content C:\ats-crawler\stdout.log -Tail 50
Get-Content C:\ats-crawler\stderr.log -Tail 50
```
...
New-NetFirewallRule -DisplayName "ATS Crawler (Tailscale only)" `
    -Direction Inbound -Action Allow -Protocol TCP -LocalPort 8901 `
    -InterfaceAlias "Tailscale"

# Deny 8901 on all other interfaces (defense in depth)
New-NetFirewallRule -DisplayName "ATS Crawler (block public)" `
...
On the API box (`root@206.189.65.177`), edit `/etc/attestseal/macbook.env`:

```bash
ssh root@206.189.65.177 "cat /etc/attestseal/macbook.env"
```
...
Sanity check:
```bash
curl https://api.attestseal.com/stats | python3 -m json.tool | grep -E "tier4|macbook|residential"
```

You should see `tier4_ok` incrementing when live checks route to residential. Hit a known-blocked merchant to force Tier 4:
```bash
curl https://api.attestseal.com/v1/check/chewy.com?refresh=true | python3 -m json.tool | head -30
```

## Step 11 -- Ongoing operations

- **Start/stop/restart:** `nssm start|stop|restart ats-crawler`
- **Reload code after a patch:** copy new `fetch_service.py` over, then `nssm restart ats-crawler`
- **Watch live logs:** `Get-Content C:\ats-crawler\stdout.log -Wait`
...
- `<GAMING_PC_TS_IP>` (ats-gaming-pc) → Spectrum
...
| Service restarts every few minutes | `CRAWLER_SHARED_SECRET` mismatch or missing env | Check `AppEnvironmentExtra` is populated: `nssm get ats-crawler AppEnvironmentExtra` |
