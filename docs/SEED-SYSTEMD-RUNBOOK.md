...
`/etc/systemd/system/ats-seed.service`:

```ini
[Unit]
Description=AttestSeal seed crawler
Documentation=https://attestseal.com
...
User=ott
Group=ott
WorkingDirectory=/opt/attestseal
Environment=ATS_ENABLE_WAYBACK_TIER=1
Environment=ATS_ENABLE_PROBE_TIER=1
ExecStart=/opt/attestseal/venv/bin/python3 /opt/attestseal/crawl_seed.py data/domains.txt --fast --workers 2 --delay 400 --resume
Restart=on-failure
RestartSec=30
StartLimitIntervalSec=600
StartLimitBurst=5
StandardOutput=append:/var/log/ats-seed.log
StandardError=append:/var/log/ats-seed.log
KillMode=mixed
KillSignal=SIGTERM
TimeoutStopSec=30
CPUQuota=80%
...
- **`Environment=ATS_ENABLE_WAYBACK_TIER=1`** and `ATS_ENABLE_PROBE_TIER=1` -- turn on Tier 5 (Wayback) and Tier 1.5 (protocol probe) in the fetch ladder. Present in the original nohup invocation; carried over verbatim.
...
Assumes you have SSH root on the seed and `/tmp/ats-seed.service` already uploaded.

```bash
# On the seed box, run:
cp /tmp/ats-seed.service /etc/systemd/system/ats-seed.service
chmod 644 /etc/systemd/system/ats-seed.service
chown root:root /etc/systemd/system/ats-seed.service

# Log file owned by ott so the service can append
touch /var/log/ats-seed.log
chown ats:ats /var/log/ats-seed.log
chmod 664 /var/log/ats-seed.log
...
systemctl enable --now ats-seed.service
sleep 4
systemctl status ats-seed.service --no-pager --lines=5
```
...
for ip in IP1 IP2 IP3 ...; do
  echo "=== $ip ==="
  scp -q /tmp/ats-seed.service root@$ip:/tmp/ats-seed.service
...
```bash
# Check service state
systemctl status ats-seed --no-pager

# Tail logs
journalctl -u ats-seed -f
# Or the raw log file:
tail -f /var/log/ats-seed.log

# Graceful stop (flushes checkpoint via SIGTERM handler)
systemctl stop ats-seed

# Restart (picks up from checkpoint)
systemctl restart ats-seed

# Disable auto-start on boot (but keep running)
systemctl disable ats-seed

# Adjust worker count or delay without rewriting the whole unit:
systemctl edit ats-seed
# then add:
# [Service]
# ExecStart=
# ExecStart=/opt/attestseal/venv/bin/python3 /opt/attestseal/crawl_seed.py data/domains.txt --fast --workers 3 --delay 300 --resume
```
...
- **API box:** attestseal.service is already systemd-managed for the web API. Different unit, different concerns.
