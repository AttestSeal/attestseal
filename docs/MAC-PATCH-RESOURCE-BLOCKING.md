...
scp "/Users/admin/Library/Mobile Documents/com~apple~CloudDocs/robots/attestseal/crawler/fetch_service.py" \
    pennyai@100.125.118.64:~/ats-crawler/fetch_service.py

# 2. Restart the crawler service to pick up the patch
ssh pennyai@100.125.118.64 "launchctl kickstart -k gui/$(id -u)/com.attestseal.crawler"
...
curl -s "https://api.attestseal.com/v1/check/chewy.com?refresh=true" | python3 -c "import sys,json;d=json.load(sys.stdin);print('trustScore:',d.get('trustScore'),'crawlability:',d.get('crawlability'),'brandTier:',d.get('brandTier'))"
```
...
```bash
ssh pennyai@100.125.118.64 "echo 'CRAWLER_BLOCK_RESOURCES=false' >> ~/ats-crawler/crawler.env && launchctl kickstart -k gui/$(id -u)/com.attestseal.crawler"
```
