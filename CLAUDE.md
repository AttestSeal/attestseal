...
1. **API droplet upgraded** -- `ats-api-1` → `ats-ap-1` (DO label), resized to 2vCPU / 2GB / 60GB ($18/mo). Longer-than-expected resize (~15 min) because disk size bumped from 25 GB to 60 GB. Services all came back clean.
2. **Litestream → Backblaze B2** -- continuous WAL-page replication to `s3://ats-db-backup/production/ott.db`, 72h granular restore, independent failure domain. Verified end-to-end (`litestream restore` match live DB exactly). Runbook at `docs/LITESTREAM-RUNBOOK.md`.
3. **Cloudflare proxy live** -- attestseal.com was already CF-registered but proxy was OFF. Turned on orange cloud for apex + www + api. SSL mode Full (strict). Page rule: `/v1/check/*` cache bypass. **Bot Fight Mode OFF** (API needs to accept default Python/Node UAs). Verified default urllib, httpx, requests, curl, node-fetch all get 200.
4. **logrotate deployed** to API + 18 seed boxes (daily, 14 days, compress, copytruncate, 100 MB size cap).
5. **Repo public** at `github.com/AttestSeal/attestseal`. Credential audit found zero leaked secrets in tracked files or git history. `SECURITY.md` with 2-day acknowledgment + 7-day first-pass + severity-tiered fix timelines.
...
- **Gotcha encountered + fixed:** systemd unit had `ATS_DB_PATH` (legacy) but new code reads `ATS_DB_PATH` → briefly wrote to empty `ots.db` next to populated `ott.db`. Patched the unit env to set both.
- **Gotcha encountered + fixed:** legacy env files live at `/etc/attestseal/*.env`; new code reads `/etc/attestseal/*.env`. Symlinked `/etc/attestseal/` → `/etc/attestseal/` families.
- **Gotcha encountered + fixed:** cron's `crawl_daily.sh` had been failing silently for 2 days because it referenced `/opt/attestseal/`. Resolved by symlinking `/opt/attestseal` → `/opt/attestseal` on the API box.
...
**Upptime status page refreshed:** repo moved to `AttestSeal/status`, 8 endpoints monitored (health, full check, stats/dataset, stats fetch counters, DID doc, landing, dashboard, methodology), `bonedoc911` assigned to incident issues for email alerts. CNAME `status.attestseal.com` configured.
...
|---|---|---|---|---|
| ats-seed-19 | CX23 | eu-central (Nuremberg) | 46.225.108.52 | v-10 bottom 37.5K |
| ats-seed-20 | CX23 | eu-central | 91.98.117.193 | v-11 bottom 37.5K |
| ats-seed-21 | CX23 | eu-central | 204.168.243.115 | v-14 bottom 37.5K |
| ats-seed-22 | CX23 | eu-central | 204.168.247.42 | v-18 bottom 37.5K |
| ats-seed-23 | CPX11 | us-west (Hillsboro) | 5.78.152.69 | v-9 Sydney range 20001-37500 |
...
for ip in <all 23 IPs>; do
  ssh root@$ip "tar czf /tmp/ats-seed-dbs.tar.gz /opt/attestseal/data/ats-*.db"
  scp root@$ip:/tmp/ats-seed-dbs.tar.gz /tmp/seed-dbs/seed-${ip//./_}.tar.gz
done
...
cd /opt/attestseal
sudo -u ott /opt/attestseal/venv/bin/python3 merge_db.py /tmp/seed-dbs/extracted/ --dry-run
# Review the plan, then:
sudo -u ott /opt/attestseal/venv/bin/python3 merge_db.py /tmp/seed-dbs/extracted/

# 3. v1.4 rescore against full merged DB
sudo -u ott /opt/attestseal/venv/bin/python3 rescore.py --dry-run
sudo -u ott /opt/attestseal/venv/bin/python3 rescore.py

# 4. Export dataset
sudo -u ott /opt/attestseal/venv/bin/python3 export_dataset.py
# Produces: exports/ats-dataset-<date>.csv, .json, .sha256
...
# GitHub: create Release on AttestSeal/attestseal with same files

# 7. Submit CrewAI PR
# Fork crewAIInc/crewAI, copy sdk/crewai-pr/attestseal_tool/ to
# lib/crewai-tools/src/crewai_tools/tools/attestseal_tool/
...
- **ats-seed-1 through ats-seed-8 on DigitalOcean** ($12/2GB droplets, Ubuntu 24.04). DO capped the account at 10 droplets, which forced the Vultr pivot.
- **ats-seed-9 through ats-seed-18 on Vultr** (same specs, spread across Sydney and other global regions for fresh WHOIS rate-limit windows + geographic proximity to country-specific WHOIS servers).
- Seeds 1-6 run **Tranco top-100K** (~16K domains each).
- Seeds 7-18 run **Tranco 100K-1M** (12 droplets × ~75K domains each).
- All boxes checkpoint-resumable, all use the same SSH key (allen@attestseal.com).
...
**Dataset export pipeline:** `server/export_dataset.py` produces CSV + JSON + SHA-256 manifest. `server/merge_db.py` merges burst DB into production with --dry-run mode. Dataset README at [dataset/README.md](attestseal/dataset/README.md) now includes `confidence` (high/medium/low) and `cautionReason` (incomplete_evidence/weak_signals/new_domain/infrastructure) fields, so agents can distinguish "CAUTION because evidence is incomplete" from "CAUTION because signals are actually weak."

**Python SDK confidence surface:** `sdk/python/attestseal/models.py` exposes `confidence` and `caution_reason`. Both CrewAI integrations ([sdk/python/attestseal/integrations/crewai.py](attestseal/sdk/python/attestseal/integrations/crewai.py) and the standalone PR-ready [sdk/crewai-pr/attestseal_tool.py](attestseal/sdk/crewai-pr/attestseal_tool.py)) surface these in tool output with decision guidance keyed to confidence level. Low-confidence CAUTION tells the agent "evidence incomplete, not necessarily bad, low-dollar OK." High-confidence CAUTION with new_domain tells the agent to confirm with user. This is the difference between a tool that blocks false positives and one that just reports a number.

**Merchant outreach email templates:** [docs/MERCHANT-OUTREACH-EMAIL.md](attestseal/docs/MERCHANT-OUTREACH-EMAIL.md). Four variants (A: weak_signals, B: incomplete_evidence, C: new_domain, D: infrastructure) plus a 7-day follow-up. Placeholder-templated, sends from alu@attestseal.com. Routing logic at bottom maps `cautionReason` to template.

**Codex critique fix bundle (shipped 2026-04-19, all 5 verified on disk):**

1. **Residential fleet loop bug** -- [server/app/fetch_escalation.py:381](attestseal/server/app/fetch_escalation.py) now `continue`s after exception inside the for-loop instead of `return None` on first failure. Tier 4 actually tries every Mac endpoint now.
2. **confidence + cautionReason cryptographically bound** -- both fields added to the signable payload in [server/app/pipeline.py:389-398](attestseal/server/app/pipeline.py) so the Ed25519 signature covers them. Agents can trust the explanation field the same way they trust the score.
3. **Confidence distinguishes gaps from weakness** -- [server/app/scoring.py:320-381](attestseal/server/app/scoring.py) rewritten to count evidence vs gaps. Reputation/SSL=0 is evidence (blocklist hit, no SSL). Content=0 with `content_scorable=False` is a gap. Domain age score=0 with `domain_age_days<0` is a gap (WHOIS failed) vs domain age score=0 with `domain_age_days>=0` which is evidence (legitimately new). Identity=0 treated as gap.
4. **`_unscorable` persisted to raw_signals** -- [server/app/pipeline.py:275](attestseal/server/app/pipeline.py) writes `"_unscorable": content_unscorable` into the content raw payload so a future rescore can preserve the incomplete_evidence vs weak_signals distinction. [server/rescore.py:197](attestseal/server/rescore.py) reads it on the way back.
5. **Completion-list selector tightened** -- [server/scripts/generate_completion_list.py:76-96](attestseal/server/scripts/generate_completion_list.py) now only requeues true evidence gaps: `content=0 AND CONTENT_UNSCORABLE flag`, `identity=0`, `age=0 with no registeredDate`, or `<3 signals populated`. Legitimately new or weak domains stay as-is.

**Upptime status page:** GitHub repo `bonedoc911/ats-status` monitors API health, landing page, dashboard every 5 minutes.

**Corporate entity:** AttestSeal, Inc. California C-Corp filed 2026-04-17. Private repo at `bonedoc911/attestseal`.
...
The real asset is the longitudinal database of trust profiles for every domain we check. Raw signal data stored separately from scores so we can re-score without re-crawling. Scoring is versioned (currently attestseal-v1.4-weights, consensus tier for Tranco top-100).

**Rebrand status (2026-04-17/18):** Code fully renamed from attestseal/ATS to attestseal/ATS in the git repo. VPS infrastructure still runs under the old paths/names (deferred to post-seed cutover). Both domains (attestseal.com and attestseal.com) serve the same API simultaneously.
...
- API: https://api.attestseal.com (FastAPI + uvicorn behind nginx, 2 workers)
- Landing page: https://attestseal.com (static HTML served by nginx)
- Internal docs: https://attestseal.com/marketing/ (knowledge base for team)
- SSL: Let's Encrypt, auto-renewing via certbot
- Service: systemd unit `attestseal.service`, auto-restarts
- Database: SQLite at /opt/attestseal/data/ots.db (1,112 domains, 2,888 raw signal records)
- Signing keys: Ed25519 at /opt/attestseal/keys/
- Tranco list: /opt/attestseal/data/tranco.csv (top 1M domains)
- Runtime dir: /opt/attestseal/logs (recreated by deploy.sh so systemd mount namespacing doesn't fail)

**Crawler box (ats-crawler-1):**
- DigitalOcean, SFO2, 167.99.172.189, Ubuntu 24.04, 1GB RAM + 2GB swap
- SSH: root@167.99.172.189 using ~/.ssh/id_ed25519
- Service: systemd unit `ats-crawler.service` running as `ott` user
...
- macOS 26.2 on Apple Silicon, Python 3.9 venv at `~/ats-crawler/`
- Tailnet address: **100.125.118.64** (on Allen's tailnet alongside the API box at 100.123.37.126)
- Service: launchd agent `~/Library/LaunchAgents/com.attestseal.crawler.plist`, `KeepAlive` on crash, stdout/stderr to `~/ats-crawler/crawler.*.log`
- Wrapper script `~/ats-crawler/start.sh` sources `crawler.env` and execs `uvicorn fetch_service:app --host 100.125.118.64 --port 8901`
- Env at `~/ats-crawler/crawler.env` (chmod 600): shares CRAWLER_SHARED_SECRET with the DO crawler box so API box auth is identical
...
scoring.py              -- attestseal-v1.3-weights, PROCEED/CAUTION/DENY, well-known brand anchor
...
ats-crawler.service     -- systemd unit with memory caps and process hardening
```
...
### Scoring Model (attestseal-v1.3-weights)
...
- `trustScore`: 0-100
- `scoringModel`: "attestseal-v1.3-weights"
...
From local Mac:
```bash
cd attestseal/deploy
bash deploy.sh root@206.189.65.177
```
Or manually:
```bash
rsync -avz --exclude='venv/' --exclude='data/' --exclude='keys/' --exclude='logs/' --exclude='__pycache__/' server/ root@206.189.65.177:/opt/attestseal/
ssh root@206.189.65.177 "mkdir -p /opt/attestseal/logs && chown -R ats:ats /opt/attestseal && systemctl restart attestseal"
```

Crawler box deploys are manual right now (rsync crawler/fetch_service.py to /opt/ats-crawler/, chown ats:ats, systemctl restart ats-crawler).

### Environment Files (not in repo)
- `/etc/attestseal/crawler.env` on API box + DO crawler box: `CRAWLER_URL=http://10.120.0.3:8901`, `CRAWLER_SHARED_SECRET=<token>`, etc
- `/etc/attestseal/decodo.env` on API box: `DECODO_HOST=gate.decodo.com`, `DECODO_PORT=7000`, `DECODO_USER=...`, `DECODO_PASS=...`
- `/etc/attestseal/macbook.env` on API box: `MACBOOK_URL=http://100.125.118.64:8901` (shares CRAWLER_SHARED_SECRET, defaults to it if no MACBOOK_SHARED_SECRET override)
- `~/ats-crawler/crawler.env` on Mac Air (user-owned, chmod 600): `CRAWLER_SHARED_SECRET=<matches VPS>`, `CRAWLER_POOL_SIZE=2`, `CRAWLER_DEFAULT_TIMEOUT_MS=25000`, `CRAWLER_BROWSER_CHANNEL=` (empty; Chrome for Testing is set via executable_path in start.sh instead)
- All VPS env files are `chmod 640` owned `root:ott` so the `ott` service user can read but not write

### Credentials
Local only at `~/.config/attestseal/credentials` (never in iCloud).
...
2. **Scoring model is versioned** (attestseal-v1.3-weights). Can be changed and re-scored from stored raw data.
...
3. **Python SDK** -- BUILT at sdk/python/. attestseal package with sync/async client, LangChain tool, CrewAI tool. Not published to PyPI yet.
4. **TypeScript SDK** -- BUILT at sdk/typescript/. @attestseal/sdk with full types, native fetch. Not published to npm yet.
...
16. **DMARC aggregate report parser** -- daily reports arriving at alu@attestseal.com from 2026-04-13 forward (DMARC p=quarantine enforcement enabled). Simple parser + dashboard next time we touch email infra.
...
19. **Reputation collectors fixed (2026-04-16/17)** -- Spamhaus DQS key (`pfg5oc35...`) wired via `/etc/attestseal/spamhaus.env`, queries routed to `dbl.dq.spamhaus.net`. All 4 sub-sources now live: Spamhaus DBL, SURBL, URLhaus, Google Safe Browsing. python-whois stderr noise suppressed via `app/whois_util.py` shared wrapper across all 4 call sites.
...
25. **Upptime status page (DEPLOYED 2026-04-17)** -- GitHub repo at `bonedoc911/ats-status`. Monitors API health, landing page, dashboard every 5 minutes. First uptime check green. Status page at `bonedoc911.github.io/ats-status` once GitHub Pages bootstrap completes.
...
- **Dathorn domain SPF/DMARC lockdown** -- only scosi.com and attestseal.com done. Allen has other domains at dathorn that need the same treatment. Blocked on: list of domains + per-domain cPanel creds OR reseller WHM API token.
...
- **Task #27 (NEW): GitHub private repo for ATS source code.** Created 2026-04-17 at `bonedoc911/attestseal` (private). Load server code, crawler code, SDKs, specs, docs. Will open-source when ready. Enables version control, PR workflow, and CI/CD pipeline.
...
- attestseal.com -- A record -> 206.189.65.177 (VPS)
- www.attestseal.com -- A record -> 206.189.65.177
- api.attestseal.com -- A record -> 206.189.65.177
- mail.attestseal.com -- A record -> 96.31.72.33 (dathorn cpanel83.gzo.com)
- MX attestseal.com -- 0 mail.attestseal.com. (points to dathorn mail server)
- SPF: `v=spf1 +mx +ip4:96.31.72.73 include:spf.gzo.com ~all` (tightened 2026-04-13; dropped `+a`)
- DMARC: `v=DMARC1; p=quarantine; rua=mailto:alu@attestseal.com; sp=quarantine; pct=100; fo=1` (enforcement enabled 2026-04-13)
...
**AttestSeal, Inc.** -- California C-Corp, filed 2026-04-17. 18575 Gale Ave Ste 278, City of Industry, CA 91748.
- Domain: attestseal.com (Cloudflare DNS, email routing to alu@scosi.com)
- API: api.attestseal.com (SSL via certbot, same VPS as attestseal.com)
- GitHub: github.com/AttestSeal (org, private repo + public status page)
- Email: alu@attestseal.com
- Rebrand in progress: code rename from attestseal/ATS to attestseal/ATS scheduled after 100K seed completes. Both domains serve the same API simultaneously.
...
Priority GTM: one reference integration with a widely-used agent SDK (LangChain, CrewAI, AutoGPT, OpenAI Swarm, Anthropic tool-use examples) that calls ATS before payments. That is the cold-start solution. The SDKs are 40% there; the other 60% is BD work with framework maintainers.
