...
def generate_verification_code() -> str:
    ...
    return f"ats-verify-{secrets.token_hex(16)}"
...
async def check_dns_verification(domain: str, expected_code: str) -> bool:
    ...
    try:
        answers = dns.resolver.resolve(f"_ats-verify.{domain}", "TXT")
...
async def check_http_verification(domain: str, expected_code: str) -> bool:
    ...
    try:
        async with httpx.AsyncClient(timeout=10, follow_redirects=True, verify=False) as client:
            resp = await client.get(f"https://{domain}/.well-known/ats-verify.txt")
...
def _get_instructions(domain: str, code: str, method: str) -> dict:
    if method == "dns":
        ...
        return {
            "method": "http",
            "steps": [
                f"Create a file at: https://{domain}/.well-known/ats-verify.txt",
                f"Contents: {code}",
