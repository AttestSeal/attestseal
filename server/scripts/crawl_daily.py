...
Usage:
    python3 crawl_daily.py              # Re-crawl ATS_DAILY_BATCH (default 200)
    python3 crawl_daily.py --batch 500  # Override batch size

Environment:
    ATS_DAILY_BATCH     How many stalest domains to re-check per run (default 200)
    ATS_DAILY_DELAY_MS  Delay between checks in ms (default 1000)
    ATS_DATA_DIR        Data directory; heartbeat lives here (default ./data)
"""
...
def data_dir() -> Path:
    return Path(os.environ.get("ATS_DATA_DIR", "./data"))
...
async def main() -> int:
    batch = int(os.environ.get("ATS_DAILY_BATCH", "200"))
    delay_ms = int(os.environ.get("ATS_DAILY_DELAY_MS", "1000"))
