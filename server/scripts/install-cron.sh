...
#   bash /opt/attestseal/scripts/install-cron.sh
#
...
TAG="# ott:crawl_daily"
LINE="0 3 * * * /opt/attestseal/scripts/crawl_daily.sh >> /opt/attestseal/logs/crawl-cron.log 2>&1 ${TAG}"
USER_NAME="ott"
...
chmod +x /opt/attestseal/scripts/crawl_daily.sh
chmod +x /opt/attestseal/scripts/crawl_daily.py
...
echo "To test immediately: sudo -u ${USER_NAME} /opt/attestseal/scripts/crawl_daily.sh"
